...
{
    private readonly List<Rectangle> freeRectangles = [];
    private readonly List<Rectangle> tempFreeRectangles = [];
    ...
    private static bool Insert(int width, int height, List<Rectangle> freeRectanglesList, ref Rectangle bestRectangle)
    {
        ...
        if (bottom is { Width: > 0, Height: > 0 })
            freeRectanglesList.Add(bottom);
        if (right is { Width: > 0, Height: > 0 })
