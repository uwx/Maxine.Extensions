using System.Diagnostics;
using System.Runtime.CompilerServices;
using System.Runtime.InteropServices;

namespace Poki.Utilities;

/// <summary>
/// <para>
/// Skip32 C# implementation.
/// 32-bit block cipher based on Skipjack.
/// </para>
///
/// <para>
/// Modified from the C# implementation:<br/>
/// https://github.com/eleven41/Eleven41.Skip32
/// </para>
///
/// <para>
/// C# implementation is an adaptation of the skip32 C implementation:<br/>
/// http://web.archive.org/web/20110719113835/http://www.qualcomm.com.au/PublicationsDocs/skip32.c
/// </para>
///
/// <para>
/// Also heavily influenced by the php adaptation:<br/>
/// https://github.com/nlenepveu/Skip32
/// </para>
/// </summary>
public class Skip32Cipher
{
    public const int KeySize = 10;
    public const int BlockSize = 4;

    private readonly byte[] _key;

    // Construct a Skip32 Cipher based on the supplied key.
    // Keys must be 10 bytes long.
    public Skip32Cipher(byte[] key)
    {
        if (key == null)
            throw new ArgumentNullException(nameof(key), "Key must not be null");
        if (key.Length != KeySize)
            throw new ArgumentOutOfRangeException(nameof(key), $"Key must be {KeySize} bytes");

        // Save for later
        _key = key;
    }

    /// <summary>
    /// Determines of the bytes of an integer need to be reversed.
    /// </summary>
    private static readonly bool IsNeedReverse = !BitConverter.IsLittleEndian;

    /// <summary>
    /// Encrypts a 32-bit value.
    /// </summary>
    /// <param name="value">Value to encrypt.</param>
    /// <returns>Encrypted value.</returns>
    public unsafe T Encrypt<T>(T value) where T : unmanaged
    {
        if (sizeof(T) != sizeof(int))
        {
            throw new ArgumentException("Type parameter must be 32-bit", nameof(T));
        }

        Span<T> valueSingleton = stackalloc T[] { value };

        var bytes = MemoryMarshal.AsBytes(valueSingleton);
        if (IsNeedReverse)
            bytes.Reverse();
        
        Debug.Assert(bytes.Length == BlockSize);
        
        Skip32(_key, bytes, true);
        if (IsNeedReverse)
            bytes.Reverse();
        
        return MemoryMarshal.Read<T>(bytes);
    }

    /// <summary>
    /// Encrypts a 4-byte array.
    /// </summary>
    /// <param name="value">Byte array to encrypt.  Must be 4 bytes.</param>
    /// <returns>4-byte encrypted array.</returns>
    public void EncryptInPlace(Span<byte> value)
    {
        if (value.Length != BlockSize)
            throw new ArgumentOutOfRangeException(nameof(value), $"Value must be {BlockSize} bytes");

        Skip32(_key, value, true);
    }

    /// <summary>
    /// Decrypts a 32-bit integer.
    /// </summary>
    /// <param name="value">Value to decrypt.</param>
    /// <returns>Decrypted value.</returns>
    public unsafe T Decrypt<T>(T value) where T: unmanaged
    {
        if (sizeof(T) != sizeof(int))
        {
            throw new ArgumentException("Type parameter must be 32-bit", nameof(T));
        }
        
        Span<T> valueSingleton = stackalloc T[] { value };

        var bytes = MemoryMarshal.AsBytes(valueSingleton);
        if (IsNeedReverse)
            bytes.Reverse();
        
        Debug.Assert(bytes.Length == BlockSize);
        
        Skip32(_key, bytes, false);
        if (IsNeedReverse)
            bytes.Reverse();
        
        return MemoryMarshal.Read<T>(bytes);
    }

    /// <summary>
    /// Decrypts a 4-byte array.
    /// </summary>
    /// <param name="value">Byte array to decrypt.  Must be 4 bytes.</param>
    /// <returns>4-byte decrypted array.</returns>
    public void DecryptInPlace(Span<byte> value)
    {
        if (value.Length != BlockSize)
            throw new ArgumentOutOfRangeException(nameof(value), $"Value must be {BlockSize} bytes");

        Skip32(_key, value, false);
    }

    // Below is taken directly from skip32.c with some minor modifications.

    private static ReadOnlySpan<byte> Ftable => new byte[]
    {
        
    };

    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    private static ushort G(byte[] key, int k, ushort w)
    {
        var g1 = (byte)((w >> 8) & 0xff);
        var g2 = (byte)(w & 0xff);

        var g3 = (byte)(Ftable[g2 ^ key[(4 * k) % KeySize]] ^ g1);
        var g4 = (byte)(Ftable[g3 ^ key[(4 * k + 1) % KeySize]] ^ g2);
        var g5 = (byte)(Ftable[g4 ^ key[(4 * k + 2) % KeySize]] ^ g3);
        var g6 = (byte)(Ftable[g5 ^ key[(4 * k + 3) % KeySize]] ^ g4);

        return (ushort)((g5 << 8) + g6);
    }

    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    private static void Skip32(byte[] key, Span<byte> data, bool isEncrypt)
    {
        const int roundCount = 2; // was 24, must be even i think
        
        // By logic, the following are known to be true
        Debug.Assert(key != null);
        Debug.Assert(key.Length == KeySize);

        int k; /* round number */
        // int i; /* round counter */
        int kstep;

        /* sort out direction */
        if (isEncrypt)
        {
            kstep = 1;
            k = 0;
        }
        else
        {
            kstep = -1;
            k = roundCount - 1;
        }

        /* pack into words */
        var wl = (ushort)((data[0] << 8) + data[1]);
        var wr = (ushort)((data[2] << 8) + data[3]);

        /* 24 feistel rounds, doubled up */
        //for (i = 0; i < roundCount / 2; ++i)
        {
            wr ^= (ushort)(G(key, k, wl) ^ k);
            k += kstep;
            wl ^= (ushort)(G(key, k, wr) ^ k);
            k += kstep;
        }

        /* implicitly swap halves while unpacking */
        data[0] = (byte)(wr >> 8);
        data[1] = (byte)(wr & 0xFF);
        data[2] = (byte)(wl >> 8);
        data[3] = (byte)(wl & 0xFF);
    }
}