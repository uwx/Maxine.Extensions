...
internal class LuaBindingIndexGenerator(
    ...
    int indentLevel = 0)
{
    public string BindingName => $"{type.LuaName}_{(isStatic ? "static_index" : "index")}";
    ...
    public string GenerateCode()
    {
        ...
        using (sb.Block())
        {
            ...
            using (sb.Block())
            {
                ...
                var isInlineArray = HasInlineArrayAttribute(type.Type);
                    ...
                    foreach (var prop in properties)
                    {
                        ...
                        using (sb.Block())
                        {
                            if (prop.HasGetter)
                            {
                                if (isStatic)
                                {
                                    sb.GeneratePushValue(prop.PropertyType, $"{type.Type.GetFullTypeName()}.{prop.Name}");
                                }
                                else
                                {
    ...
    {
        var inlineArrayLength = isInlineArray ? GetInlineArrayLength(type.Type) : null;
        var inlineArrayElementType = isInlineArray ? GetInlineArrayElementType(type.Type) : null;
    ...
    {
        return type.GetCustomAttribute<InlineArrayAttribute>() != null;
    }
    ...
    {
        return type.GetCustomAttribute<InlineArrayAttribute>()?.Length;
    }
