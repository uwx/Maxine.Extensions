...
public class LuaBindingGenerator(Assembly assembly, string @namespace)
{
    ...
    /// <summary>
    /// Check if a method has any byref parameters (ref, out, ptr*).
    /// </summary>
    private static bool HasByRefParameters(MethodBase method)
    {
        return method.GetParameters().Any(p => p.IsOut || (p.ParameterType.IsByRef && !p.IsOut && !p.IsIn) || IsRefStruct(p.ParameterType) || p.ParameterType.IsPointer);
    }
    ...
    private static bool IsBlacklistedType(Type type)
    {
        return type == typeof(Array) ||
               type == typeof(MemoryHandle) ||
               type == typeof(Type) ||
               type.Namespace?.StartsWith("System.Reflection") == true ||
               type.Namespace?.StartsWith("System.Runtime.CompilerServices") == true ||
               type.Namespace?.StartsWith("System.Runtime.Serialization") == true;
    }
    ...
    private void GenerateLuaStubsFile(string outputDirectory, Dictionary<TypeInfo, DiscoveredKind> types)
    {
        _sb.Clear();
        _indent = 0;
        ...
        AppendLine("-- Generated by Maxine.Extensions.Lua.SourceGenerator");
        AppendLine();

        foreach (var (typeInfo, kind) in types)
        {
            GenerateLuaTypeStub(typeInfo, kind);
            AppendLine();
        }
    ...
    private void GenerateTypeScriptDeclarationsFile(string outputDirectory, Dictionary<TypeInfo, DiscoveredKind> types)
    {
        _sb.Clear();
        _indent = 0;

        AppendLine("// @ts-nocheck");
        AppendLine("// <auto-generated />");
        AppendLine("// TypeScript declarations for TypeScriptToLua");
        AppendLine("// This file is auto-generated by Maxine.Extensions.Lua.SourceGenerator.");
        AppendLine();

        foreach (var (typeInfo, kind) in types)
        {
            GenerateTypeScriptTypeDeclaration(typeInfo, kind);
            AppendLine();
        }

        var outputPath = Path.Combine(outputDirectory, "LuaBindings.d.ts");
        File.WriteAllText(outputPath, _sb.ToString());
    }

    private void GenerateTypeScriptTypeDeclaration(TypeInfo typeInfo, DiscoveredKind kind)
    {
        ...
        if (isArray)
        {
            ...
            using (Indent())
            {
                if (rank == 1)
                {
                    AppendLine("/** @customName new */");
                    AppendLine($"static inst(length: number): {luaName};");
                }
                else
                {
                    AppendLine("/** @customName new */");
                    var dims = string.Join(", ", Enumerable.Range(0, rank).Select(i => $"dim{i}: number"));
                    AppendLine($"static inst({dims}): {luaName};");
                }
            }
            using (Indent())
            {
                if (rank == 1)
                {
                    AppendLine($"[index: number]: {tsElementType};");
                    AppendLine($"readonly length: number;");
                }
                else
                {
                    AppendLine($"get(...indices: number[]): {tsElementType};");
        ...
        {
            var baseLuaName = GetSafeTypeName(baseType);
            extendsClause = $" extends {baseLuaName}";
        }
        ...
        AppendLine($"declare class {luaName} {{");
        if ((constructors.Count > 0 || staticMethods.Count > 0 || isStruct) && HasInitializeMethod(kind))
        {
            using (Indent())
            {
                // Constructors
                ...
                {
                    AppendLine("/** @customName new */");
                    AppendLine($"static inst(): {luaName};");
                }
                foreach (var ctor in constructors)
                {
                    ...
                    AppendLine($"static inst({paramStr}): {luaName};");
                }
                ...
                foreach (var method in staticMethods)
                {
                    ...
                    var parameters = method.GetParameters()
                        ...
                        .ToList();
                    var paramStr = string.Join(", ", parameters);
                    var returnType = method.ReturnType == typeof(void) ? "void" : GetTypeScriptTypeName(method.ReturnType);
                    AppendLine($"static {luaMethodName}({paramStr}): {returnType};");
                }
            }
        }
        ...
        using (Indent())
        {
            ...
            var props = type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
                .Where(p => !p.GetIndexParameters().Any() && !HasAttribute(p, nameof(LuaHiddenAttribute)) && !HasRefReturn(p))
                .ToList();

            foreach (var prop in props)
            {
                var luaPropertyName = GetLuaPropertyName(prop);
                var tsType = GetTypeScriptTypeName(prop.PropertyType);
                var readonlyMod = !prop.CanWrite || IsInitOnly(prop) ? "readonly " : "";
                AppendLine($"{readonlyMod}{luaPropertyName}: {tsType};");
            }

            // Fields
            var fields = type.GetFields(BindingFlags.Public | BindingFlags.Instance)
                .Where(f => !HasAttribute(f, nameof(LuaHiddenAttribute)) && !IsRefStruct(f.FieldType))
                .ToList();

            foreach (var field in fields)
            {
                var luaFieldName = GetLuaFieldName(field);
                var tsType = GetTypeScriptTypeName(field.FieldType);
            ...
            foreach (var method in methods)
            {
                ...
                var paramStr = string.Join(", ", parameters);
                var returnType = method.ReturnType == typeof(void) ? "void" : GetTypeScriptTypeName(method.ReturnType);
                AppendLine($"{luaMethodName}({paramStr}): {returnType};");
            }
    ...
    private void GenerateLuaTypeStub(TypeInfo typeInfo, DiscoveredKind kind)
    {
        var type = typeInfo.Type;
        var luaName = typeInfo.LuaName;
        var isStruct = type.IsValueType && !type.IsPrimitive && !type.IsEnum;
        ...
        if (HasInitializeMethod(kind) || isArray)
        {
                ...
                if (prop.CanRead && prop.GetIndexParameters() is { Length: > 0 } indexParameters)
                {
                    ...
                    else
                    {
                        var indexTypes = string.Join(", ",
                            indexParameters.Select(p => GetLuaStubTypeName(p.ParameterType)));
                        AppendLine($"---@field [[{indexTypes}]] {propType}");
                    }
                }
                else
    ...
    private void GenerateInitializeFile(string outputDirectory, Dictionary<TypeInfo, DiscoveredKind> types)
    {
        ...
        AppendLine();
        AppendLine("using LuaJIT;");
        AppendLine("using static LuaJIT.Methods;");
        AppendLine();
    ...
    private void GenerateIndexMethod(Type type, string safeName, bool isStruct, string fullTypeName)
    {
        ...
        using (Block())
        {
            ...
            var isArray = type.IsArray;
            var indexers = type.GetProperties(BindingFlags.Public | BindingFlags.Instance | BindingFlags.FlattenHierarchy)
