...
#pragma warning disable CA1822
    public void Append([InterpolatedStringHandlerArgument("")] scoped in ValueAppendInterpolatedStringHandler handler)
...
#pragma warning restore CA1822
        ...
        [InterpolatedStringHandlerArgument("", nameof(provider))] scoped in ValueAppendInterpolatedStringHandler handler
    )
    {
        this = handler.StringBuilder;
    }
    ...
    /// <returns>A reference to this instance after the append operation has completed.</returns>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void AppendLine([InterpolatedStringHandlerArgument("")] scoped in ValueAppendInterpolatedStringHandler handler)
    {
        this = handler.StringBuilder;
        AppendLine();
    }
    ...
    /// <summary>Provides a handler used by the language compiler to append interpolated strings into <see cref="System.Text.StringBuilder"/> instances.</summary>
    ...
    public ref struct ValueAppendInterpolatedStringHandler
    {
        ...
        internal ValueStringBuilder StringBuilder;
        ...
        /// <summary>Creates a handler used to append an interpolated string into a <see cref="System.Text.StringBuilder"/>.</summary>
        ...
        // a copy is made here because roslyn will not pass stringBuilder by 'ref', and 'in' causes a compiler error due to exposing out ot scope
        // instead we copy it back when we're done
        // this is a bit slow, but not terrible
        public unsafe ValueAppendInterpolatedStringHandler(int literalLength, int formattedCount, scoped ValueStringBuilder stringBuilder)
        {
        ...
        /// <param name="provider">An object that supplies culture-specific formatting information.</param>
        /// <remarks>This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used directly.</remarks>
        public unsafe ValueAppendInterpolatedStringHandler(int literalLength, int formattedCount, scoped ValueStringBuilder stringBuilder, IFormatProvider? provider)
        {
            StringBuilder = stringBuilder;
            _provider = provider;
        ...
        public void AppendFormatted<T>(T value)
        {
            ...
            else if (value is IFormattable)
            {
                ...
                if (typeof(T).IsEnum)
                {
                    if (TryFormatUnconstrained(default!, value, StringBuilder.RemainingCurrentChunk, out var charsWritten))
                    {
                        StringBuilder._pos += charsWritten;
                    }
                    else
                    {
                ...
                {
                    var destination = StringBuilder.RemainingCurrentChunk;
                ...
                {
                    StringBuilder.Append(((IFormattable)value).ToString(format: null, _provider)); // constrained call avoiding boxing for value types
                }
            }
        ...
        public void AppendFormatted<T>(T value, string? format)
        {
            ...
            else if (value is IFormattable formattable)
            {
                ...
                if (typeof(T).IsEnum)
                {
                    if (TryFormatUnconstrained(default!, value, StringBuilder.RemainingCurrentChunk, out int charsWritten, format))
                    {
                        StringBuilder._pos += charsWritten;
                    }
                    else
                ...
                {
                    StringBuilder.Append(formattable.ToString(format, _provider)); // constrained call avoiding boxing for value types
                }
            }
            else if (value is not null)
            {
        ...
        public void AppendFormatted<T>(T value, int alignment, string? format)
        {
            ...
            else if (alignment < 0)
            {
                ...
                AppendFormatted(value, format);
                var paddingRequired = -alignment - (StringBuilder.Length - start);
                if (paddingRequired > 0)
                {
                    StringBuilder.Append(' ', paddingRequired);
                }
            }
            else
            {
        ...
        public void AppendFormatted(ReadOnlySpan<char> value) => StringBuilder.Append(value);
        ...
        private void AppendCustomFormatter<T>(T value, string? format)
        {
            ...
            {
                StringBuilder.Append(formatter.Format(format, value, _provider));
            }
        }
    }
