#if ON
// https://github.com/dotnet/aspnetcore/blob/f26b79eb34d13648bedc5ec98e2151f8c55db978/src/Shared/Buffers.MemoryPool/UnmanagedBufferAllocator.cs
...
internal unsafe struct UnmanagedBufferAllocator : IDisposable
{
    ...
    private List<UnmanagedMemoryManager<byte>> _currentAlloc = new();
    ...
    private byte* Alloc(int size)
    {
        ...
        var newBlock = (byte*)NativeMemory.Alloc((nuint)(size));
        
        newBlock[]

        new UnmanagedMemoryManager<byte>((byte*)newBlock, );
