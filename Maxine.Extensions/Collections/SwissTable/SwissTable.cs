...
using static Maxine.Extensions.Collections.SwissTableHelper;
...
public class SwissTable<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue> where TKey : notnull
{
    // Term define:
    ...
    public SwissTable() : this(0) { }

    public SwissTable(IEqualityComparer<TKey>? comparer) : this(0, comparer) { }

    public SwissTable(int capacity, IEqualityComparer<TKey>? comparer = null)
    {
    ...
    {
        if (comparer is not null && !ReferenceEquals(comparer, EqualityComparer<TKey>.Default)) // first check for null to avoid forcing default comparer instantiation unnecessarily
        {
            _comparer = comparer;
        }
        ...
        {
            var stringComparer = NonRandomizedStringEqualityComparer.GetStringComparer(_comparer);
    ...
    public SwissTable(IDictionary<TKey, TValue> dictionary, IEqualityComparer<TKey>? comparer = null)
    {
    ...
    public SwissTable(IEnumerable<KeyValuePair<TKey, TValue>> collection, IEqualityComparer<TKey>? comparer = null)
    {
    ...
    public IEqualityComparer<TKey> Comparer
    {
        get
        {
            if (typeof(TKey) == typeof(string))
            {
                return _comparer is NonRandomizedStringEqualityComparer c ? (IEqualityComparer<TKey>)c.GetUnderlyingEqualityComparer() : _comparer ?? EqualityComparer<TKey>.Default;
            }
            else
            {
    ...
    private void CloneFromCollection(IEnumerable<KeyValuePair<TKey, TValue>> collection)
    {
        ...
        {
            var source = (SwissTable<TKey, TValue>)collection;
    ...
    public TValue this[TKey key]
    {
        get
        {
            ref var entry = ref FindBucket(key);
    ...
    object? IDictionary.this[object key]
    {
        get
        {
            ...
            {
                ref var entry = ref FindBucket((TKey)key);
                if (!Unsafe.IsNullRef(ref entry))
                {
                    return entry.Value;
                }
    ...
    {
        ref var bucket = ref FindBucket(keyValuePair.Key);
    ...
    {
        /// <inheritdoc cref="CollectionsMarshal.GetValueRefOrAddDefault{TKey, TValue}(SwissTable{TKey, TValue}, TKey, out bool)"/>
    ...
    private void GrowWorker(int idealEntryLength)
    {
        ...
        var oldCtrls = rawTable._controls;
        var oldEntries = rawTable._entries;
        Entry[] newEntries = newTable._entries;
        var length = rawTable._entries.Length;
        // TODO: Maybe we could use IBITMASK to accllerate
    ...
    public int EnsureCapacity(int capacity)
    {
        ...
        var currentCapacity = rawTable._count + rawTable._growth_left;
    ...
    private TValue erase(int index)
    {
        ...
        var res = rawTable._entries[index].Value;
