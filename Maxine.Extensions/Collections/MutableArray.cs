...
/// <see cref="ImmutableArray{T}"/>, but with no guarantees of immutability. Implementation is mostly a hodgepodge of
/// code from ImmutableArray adapted to work here.
/// </summary>
/// <typeparam name="T">The type of element stored by the array.</typeparam>
public readonly struct MutableArray<T> : IImmutableList<T>, IList<T>, IList, IStructuralEquatable, IStructuralComparable
{
    public readonly T[] BackingArray;
    
    int IReadOnlyCollection<T>.Count => BackingArray.Length;
    int ICollection<T>.Count => BackingArray.Length;
    ...
    /// </summary>
    public bool IsEmpty => BackingArray == null || BackingArray.Length == 0;
    
    /// <summary>
    /// Gets a value indicating whether this struct was initialized without an actual array instance.
    /// </summary>
    public bool IsDefault => BackingArray == null;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public MutableArray(T[] backingArray)
    {
        BackingArray = backingArray;
    }

    public IEnumerator<T> GetEnumerator() => ((IEnumerable<T>)BackingArray).GetEnumerator();
    IEnumerator IEnumerable.GetEnumerator() => BackingArray.GetEnumerator();

    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator MutableArray<T>(T[] array) => new(array);
    
    [Pure]
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static implicit operator T[](MutableArray<T> mutableArray) => mutableArray.BackingArray;

    #region ImmutableArray<T> Borrowed Impl
    
    [Pure]
    public ReadOnlySpan<T> AsSpan() => new(BackingArray);

    [Pure]
    public ReadOnlyMemory<T> AsMemory() => new(BackingArray);
    ...
    public int IndexOf(T item, int startIndex, int count, IEqualityComparer<T>? equalityComparer)
    {
        ...
        {
            return Array.IndexOf(self.BackingArray, item, startIndex, count);
        }
        else
        {
            for (int i = startIndex; i < startIndex + count; i++)
            {
                if (equalityComparer.Equals(self.BackingArray[i], item))
                {
                    return i;
                }
            }
    ...
    public int LastIndexOf(T item, int startIndex, int count, IEqualityComparer<T>? equalityComparer)
    {
        ...
        if (ReferenceEquals(equalityComparer, EqualityComparer<T>.Default))
        {
            return Array.LastIndexOf(self.BackingArray, item, startIndex, count);
        }
        else
        {
            for (int i = startIndex; i >= startIndex - count + 1; i--)
            {
                if (equalityComparer.Equals(item, self.BackingArray[i]))
                {
                    return i;
                }
            }
    ...
    [Pure]
    public MutableArray<T> Insert(int index, T item)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        ...
        if (index != 0)
        {
            Array.Copy(self.BackingArray, tmp, index);
        }
        if (index != self.Length)
        {
            Array.Copy(self.BackingArray, index, tmp, index + 1, self.Length - index);
        }

        return new MutableArray<T>(tmp);
    }
    ...
    [Pure]
    public MutableArray<T> InsertRange(int index, IEnumerable<T> items)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        Requires.Range(index >= 0 && index <= self.Length, nameof(index));
        ...
        if (index != self.Length)
        {
            Array.Copy(self.BackingArray, index, tmp, index + count, self.Length - index);
        }
    ...
    /// <returns>The new immutable collection.</returns>
    [Pure]
    public MutableArray<T> InsertRange(int index, MutableArray<T> items)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        ...
        return items.IsEmpty ? self : self.InsertSpanRangeInternal(index, items.AsSpan());
    }
    ...
    /// <returns>A new array.</returns>
    [Pure]
    public MutableArray<T> Add(T item)
    {
        var self = this;
        if (self.IsEmpty)
        {
    ...
    [Pure]
    public MutableArray<T> AddRange(IEnumerable<T> items)
    {
        var self = this;
    ...
    /// <returns>A new list with the elements added.</returns>
    [Pure]
    public MutableArray<T> AddRange(T[] items, int length)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        ArgumentNullException.ThrowIfNull(items, nameof(items));
        ...
        Array.Copy(self.BackingArray, tmp, self.Length);
    ...
    /// <returns>A new list with the elements added.</returns>
    [Pure]
    public MutableArray<T> AddRange<TDerived>(TDerived[] items) where TDerived : T
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        ArgumentNullException.ThrowIfNull(items, nameof(items));
    ...
    public MutableArray<T> AddRange(MutableArray<T> items, int length)
    {
        var self = this;
        Requires.Range(length >= 0, nameof(length));

        return items.BackingArray != null ? self.AddRange(items.BackingArray, length) : self;
    }
    ...
    [Pure]
    [SuppressMessage("ReSharper", "ConditionIsAlwaysTrueOrFalseAccordingToNullableAPIContract")]
    public MutableArray<T> AddRange<TDerived>(MutableArray<TDerived> items) where TDerived : T
    {
        var self = this;
        return items.BackingArray != null ? self.AddRange(items.BackingArray) : self;
    }
    ...
    /// <returns>A new list with the elements added.</returns>
    [Pure]
    public MutableArray<T> AddRange(MutableArray<T> items)
    {
        var self = this;
        return self.InsertRange(self.Length, items);
    }
    ...
    [Pure]
    public MutableArray<T> SetItem(int index, T item)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        Requires.Range(index >= 0 && index < self.Length, nameof(index));
    ...
    [Pure]
    public MutableArray<T> Replace(T oldValue, T newValue)
    {
    ...
    /// <exception cref="ArgumentException">Thrown when the old value does not exist in the list.</exception>
    [Pure]
    public MutableArray<T> Replace(T oldValue, T newValue, IEqualityComparer<T>? equalityComparer)
    {
        var self = this;
        int index = self.IndexOf(oldValue, 0, self.Length, equalityComparer);
        if (index < 0)
        {
            throw new ArgumentException("Cannot find old value", nameof(oldValue));
        }
    ...
    /// <returns>The new array.</returns>
    [Pure]
    public MutableArray<T> Remove(T item)
    {
    ...
    /// <returns>The new array.</returns>
    [Pure]
    public MutableArray<T> Remove(T item, IEqualityComparer<T>? equalityComparer)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
    ...
    /// <returns>The new array.</returns>
    [Pure]
    public MutableArray<T> RemoveAt(int index)
    {
    ...
    /// <returns>The new array.</returns>
    [Pure]
    public MutableArray<T> RemoveRange(int index, int length)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        Requires.Range(index >= 0 && index <= self.Length, nameof(index));
        ...
        T[] tmp = new T[self.Length - length];
        Array.Copy(self.BackingArray, tmp, index);
        Array.Copy(self.BackingArray, index + length, tmp, index, self.Length - index - length);
        return new MutableArray<T>(tmp);
    }
    ...
    /// </returns>
    [Pure]
    public MutableArray<T> RemoveRange(IEnumerable<T> items)
    {
    ...
    /// </returns>
    [Pure]
    public MutableArray<T> RemoveRange(IEnumerable<T> items, IEqualityComparer<T>? equalityComparer)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        ArgumentNullException.ThrowIfNull(items, nameof(items));
    ...
    /// </returns>
    [Pure]
    public MutableArray<T> RemoveRange(MutableArray<T> items)
    {
        return RemoveRange(items, EqualityComparer<T>.Default);
    }
    ...
    /// </returns>
    [Pure]
    public MutableArray<T> RemoveRange(MutableArray<T> items, IEqualityComparer<T>? equalityComparer)
    {
        ArgumentNullException.ThrowIfNull(items.BackingArray, nameof(items));
    ...
    /// </returns>
    [Pure]
    public MutableArray<T> RemoveAll(Predicate<T> match)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        ...
        for (int i = 0; i < self.BackingArray.Length; i++)
        {
    ...
    [Pure]
    public MutableArray<T> Sort(Comparison<T> comparison)
    {
    ...
    [Pure]
    public MutableArray<T> Sort(IComparer<T>? comparer)
    {
        var self = this;
    ...
    /// <param name="comparer">The comparer to use in sorting. If <c>null</c>, the default comparer is used.</param>
    [Pure]
    public MutableArray<T> Sort(int index, int count, IComparer<T>? comparer)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        ...
        if (count > 1)
        {
            ...
            {
                if (comparer.Compare(self.BackingArray[i - 1], self.BackingArray[i]) > 0)
                {
                    outOfOrder = true;
                    break;
                }
    ...
    /// <returns>A new list with the elements added.</returns>
    [Pure]
    public MutableArray<T> AddRange(ReadOnlySpan<T> items)
    {
        var self = this;
        return self.InsertRange(self.Length, items);
    }
    ...
    /// <returns>A new list with the elements added.</returns>
    [Pure]
    public MutableArray<T> AddRange(params T[] items)
    {
        var self = this;
        return self.InsertRange(self.Length, items);
    }
    ...
    [Pure]
    public ReadOnlySpan<T> AsSpan(int start, int length) => new(BackingArray, start, length);
    ...
    /// <returns>The new immutable collection.</returns>
    [Pure]
    public MutableArray<T> InsertRange(int index, T[] items)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        Requires.Range(index >= 0 && index <= self.Length, nameof(index));
        ...
        return self.IsEmpty ? new MutableArray<T>(items) : self.InsertSpanRangeInternal(index, items);
    }
    ...
    /// <returns>The new immutable collection.</returns>
    [Pure]
    public MutableArray<T> InsertRange(int index, ReadOnlySpan<T> items)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        Requires.Range(index >= 0 && index <= self.Length, nameof(index));
    ...
    /// </returns>
    [Pure]
    public MutableArray<T> RemoveRange(ReadOnlySpan<T> items, IEqualityComparer<T>? equalityComparer = null)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
    ...
    /// </returns>
    [Pure]
    public MutableArray<T> RemoveRange(T[] items, IEqualityComparer<T>? equalityComparer = null)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
    ...
    [Pure]
    public MutableArray<T> Slice(int start, int length)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        return self.BackingArray[start..(start+length)];
    }
    ...
    /// </summary>
    /// <param name="destinationArray">The one-dimensional <see cref="Array"/> that is the destination of the elements copied from <see cref="ICollection"/>. The <see cref="Array"/> must have zero-based indexing.</param>
    /// <param name="index">The zero-based index in <paramref name="destinationArray"/> at which copying begins.</param>
    void ICollection.CopyTo(Array destinationArray, int index)
    {
        var self = this;
        self.ThrowInvalidOperationIfNotInitialized();
        Array.Copy(self.BackingArray, 0, destinationArray, index, self.Length);
    }
    ...
    bool IStructuralEquatable.Equals(object? other, IEqualityComparer comparer)
    {
        ...
        if (otherArray == null)
        {
            if (other is MutableArray<T> theirs)
            {
                otherArray = theirs.BackingArray;

                if (self.BackingArray == null && otherArray == null)
                {
                    return true;
                }
                else if (self.BackingArray == null)
                {
                    return false;
                }
            }
        ...
        IStructuralEquatable ours = self.BackingArray;
    ...
    int IStructuralComparable.CompareTo(object? other, IComparer comparer)
    {
        ...
        if (otherArray == null)
        {
            if (other is MutableArray<T> theirs)
            {
                ...
                else if (self.BackingArray == null ^ otherArray == null)
                {
                    throw new ArgumentException("ArrayInitializedStateNotEqual", nameof(other));
                }
            }
        }
        ...
        if (otherArray != null)
        {
            IStructuralComparable? ours = self.BackingArray;
            if (ours == null)
            {
                throw new ArgumentException("ArrayInitializedStateNotEqual", nameof(other));
            }
        ...
        throw new ArgumentException("ArrayLengthsNotEqual", nameof(other));
    }
    ...
    /// <returns>The new array.</returns>
    private MutableArray<T> RemoveAtRange(ICollection<int> indicesToRemove)
    {
        var self = this;
        self.ThrowNullRefIfNotInitialized();
        ArgumentNullException.ThrowIfNull(indicesToRemove, nameof(indicesToRemove));
        ...
        foreach (int indexToRemove in indicesToRemove)
        {
            ...
            Array.Copy(self.BackingArray, copied + removed, newArray, copied, copyLength);
            removed++;
            copied += copyLength;
            lastIndexRemoved = indexToRemove;
        }

        Array.Copy(self.BackingArray, copied + removed, newArray, copied, self.Length - (copied + removed));

        return new MutableArray<T>(newArray);
    }

    private MutableArray<T> InsertSpanRangeInternal(int index, ReadOnlySpan<T> items)
    {
        Debug.Assert(BackingArray != null);
        ...
        if (index != 0)
        {
            Array.Copy(BackingArray, tmp, index);
        }
        items.CopyTo(new Span<T>(tmp, index, items.Length));
        if (index != Length)
        {
            Array.Copy(BackingArray, index, tmp, index + items.Length, Length - index);
        }

        return new MutableArray<T>(tmp);
    }
    
    void ICollection<T>.CopyTo(T[] destinationArray, int arrayIndex)
    {
        var self = this;
        self.ThrowInvalidOperationIfNotInitialized();
        Array.Copy(self.BackingArray, 0, destinationArray, arrayIndex, self.Length);
    }
    #endregion

    #region ImmutableArray<T> Utility Methods
    ...
    internal void ThrowNullRefIfNotInitialized()
    {
        ...
        // extra expense.
        _ = BackingArray.Length;
    }

    /// <summary>
    /// Throws an <see cref="InvalidOperationException"/> if the <see cref="BackingArray"/> field is null, i.e. the
    ...
    private void ThrowInvalidOperationIfNotInitialized()
    {
        if (IsDefault)
        {
            throw new InvalidOperationException("Invalid operation on default array.");
        }
    }
...
// from System.Collections.Immutable.ImmutableExtensions
internal static class ImmutableExtensions
{
    ...
    internal static bool TryCopyTo<T>(this IEnumerable<T> sequence, T[] array, int arrayIndex)
    {
        ...
        if (sequence is IList<T>)
        {
            ...
            if (sequence is MutableArray<T> immutable)
            {
                Array.Copy(immutable.BackingArray, 0, array, arrayIndex, immutable.Length);
                return true;
            }
        }
