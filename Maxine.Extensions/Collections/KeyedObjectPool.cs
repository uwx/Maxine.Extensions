...
internal readonly struct ConcurrentQueueMultiDictionary<TKey, TValue> : IDictionary<TKey, ConcurrentQueue<TValue>>, IEnumerable<KeyValuePair<TKey, TValue>> where TKey : notnull
{
    ...
    public void Add(TKey key, TValue value)
    {
        _dictionary.AddOrUpdate(
            ...
            static (_, value1) =>
            {
                var concurrentQueue = new ConcurrentQueue<TValue>(); // this is Not Slower than new ConcurrentQueue(IEnumerable) because we are not enqueuing >InitialSegmentLength entries at once
