...
public class LuaBindingGenerator(Assembly assembly, string @namespace)
{
    ...
    private void GenerateTypeScriptDeclarationsFile(string outputDirectory, List<TypeInfo> types)
    {
        ...
        foreach (var typeInfo in types)
        {
            GenerateTypeScriptTypeDeclaration(typeInfo);
            AppendLine();
        }
    ...
    private void GenerateTypeScriptTypeDeclaration(TypeInfo typeInfo)
    {
        ...
        AppendLine($"declare class {luaName} {{");
        if (constructors.Count > 0 || staticMethods.Count > 0 || isStruct)
        {
            using (Indent())
            {
                // Constructors
